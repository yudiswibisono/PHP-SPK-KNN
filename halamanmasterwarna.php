							    ...
							    <?php
									...
									while ($row=mysqli_fetch_assoc($res))
									{
										...
										<tr>
										  ...
										  <td>
										    <div class="col-sm-10">
												<select onchange="ganti(<?php echo $row["id"];?>);" class="form-control" required="" name="namawarna" id="namawarna<?php echo $row["id"];?>">
												<option>Belum ada group warna</option>
													<?php
														
														$q2="SELECT * from warna order by nama_warna";
														
														$res2=mysqli_query($conn,$q2);
														while ($rowCmbBox=mysqli_fetch_assoc($res2))
														{
															$selected="";
															if ($rowCmbBox["id_warna"]===$row['id_warna'])
															{
																$selected="selected";
															}
															?>
															  <option <?php echo $selected;?> value="<?php echo $rowCmbBox["id_warna"];?>">
																<?php
																	echo $rowCmbBox["nama_warna"];
																?>
															  </option>
															<?php
														}
