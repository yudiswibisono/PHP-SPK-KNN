<?php
	...
	if (isset($_GET["nama"]))
	{
		$uname1=mysqli_real_escape_string($conn,$_GET["nama"]);
	}
	$uharga="";
	if (isset($_GET["harga"]))
	{
	...
	if (isset($_GET["cc"]))
	{
		$ucc=mysqli_real_escape_string($conn,$_GET["cc"]);
	}
	$utransmisi="";
	...
	?>
				...
				<div class="panel panel-primary">
					<div class="card card-outline-secondary">
						<div class="card-header"style="background-color: lightgreen; color: black; text-align: center;">
							<h3 class="mb-0">Cari Sepeda Motor Baru</h3>
						</div>
						<?php
							if (isset($_GET["w"]))
							{
						...
						<div class="card-body" style="opacity:0.9;">
							<form autocomplete="off" class="form" id="formMobilBekas" name="formLogin" role="form">
								<div class="form-group row">
									...
									<div class="col-sm-10">
									  <input class="form-control" placeholder="Masukkan nama/merek sepeda motor" id="uname1" value="<?php echo $uname1;?>" name="nama" type="text" value="">
									</div>
								</div>	
								
								<div class="form-group row">
									<label for="staticEmail" class="col-sm-2 col-form-label" style="text-align:right">Tipe Motor : </label>
									<div class="col-sm-10">
										<select class="form-control" name="tipe" id="utipe">
										<option value="" disabled selected>Pilih tipe motor</option>
										<option></option>
											<?php
												$q="SELECT distinct jenis FROM motorbaru
													ORDER BY jenis ASC;
													";
												...
												while ($row=mysqli_fetch_assoc($res))
												{
													...
													?>
													  <option <?php echo $selected;?> value="<?php echo $row["jenis"];?>">
														<?php
															echo $row["jenis"];
														?>
													  </option>
								...
								<div class="form-group row">
									...
									<div class="col-sm-10">
									<input class="form-control" placeholder="Masukkan harga sepeda motor yang diinginkan " value="<?php echo $uharga;?>" onkeyup="komaharga();" id="harga" name="harga" type="text">
									<label id="lharga"></label>
									</div>
								</div>
								<div class="form-group row">
									<label for="uname1" class="col-sm-2 col-form-label" style="text-align:right">Kapasitas mesin : </label> 
									<div class="col-sm-10">
									<input class="form-control" placeholder="Masukkan kapasitas mesin sepeda motor" value="<?php echo $ucc;?>" id="cc" name="cc" type="text">
									<label id="lharga"></label>
									</div>
								</div>
								<?php
							...
							?>
								<div class="card card-outline-secondary">
										<form autocomplete="off" class="form" id="formMobilBekas" name="formLogin" role="form">
											...
											<a href="riwayatmotorbaru.php" class="btn btn-primary btn-lg active" role="button" aria-pressed="true">History Pencarian</a>
										</form>
								</div>
							<?php
							}
			...
			<div class="container" style="margin-top:10px">
				<div class="row">
					<?php
					    ...
					    $fieldNormalisasi=array("harga","cc");
						$dataN=array();
						$dataV=array();
						$minVal=0;
						$maxVal=1;
						...
						if ($utipe!="")
						{
							$syarat=$syarat." AND m.jenis = '$utipe' ";
						}
						if ($utransmisi!="")
						{
							$syarat=$syarat." AND tt.id_transmisi=$utransmisi";
						}
						$q="SELECT m.nama,m.harga,m.cc,m.jenis,t.transmisi as transmisi,m.gambar,m.url,m.tipe ".
						   "FROM motorbaru m ".
						...
						if ($cc!="")
						{
							...
							if (count($dataN)>=1)
							{
								$maxCc=$dataN[0]["max_cc"];
								$minCc=$dataN[0]["min_cc"];
							}
							...
							if ($pembagi!=0)
							{
								  $normalisasicc=($cc-$minCc)/$pembagi;
							}
						}
						...
						for ($i=0;$i<count($dataN);$i++)
						{
							...
							$jarakCc=$normalisasicc-$dataN[$i]["ncc"];
								...
								?> 
									  <div style="height:500px;" class="col-md-4">
										<div class="row" style="border-style: solid;background-color: white;opacity:0.97;">
											...
											<?php
													$gambar=$row["gambar"];
													if (!file_exists("crawler/motorbaru/gambar/".$gambar))
													{
														$gambar="nopic.jpg";
													}
												?>
												
												<img src="crawler/motorbaru/gambar/<?php echo $gambar;?>" style="border-style: solid;display:block;margin:auto;height:150px">
												...
												<table class="table table-bordered">
												   ...
												   <tr>
													 <td bgcolor="#ffffff" style="font-size:15px;text-align:left">
													   <?php
														echo "<b>Harga :</b> Rp.". number_format($row["harga"]). $row["nharga"];//." ".$Dharga;//. $row["ncc"];
													   ?>
													 </td>
												   </tr>
												   <tr>
													 <td bgcolor="#ffffff" style="font-size:15px;text-align:left">
													   <?php
														echo "<b>Jenis : </b>". $row["jenis"]//." ".$Dtahun;
													   ?>
													 </td>
												   </tr>
												   <tr>
												   ...
												   <tr>
													 <td bgcolor="#ffffff" style="font-size:15px;text-align:left">
													   <?php
														echo "<b>Kapasitas Mesin : </b>". $row["cc"] . "cc"//. $row["ncc"];//." ".$row["nwarna"];
													   ?>
													 </td>
												   </tr>
													<tr>
			...
			<div class="col-md-12" >
				<nav aria-label="Page navigation example" style="margin-bottom:20px">
						...
						<li class="page-item " >
							<a class="page-link" href="motorbaru.php?start=0&kepala=-1&nama=<?php echo $uname1;?>&tipe=<?php echo $utipe;?>&harga=<?php echo $harga;?>&cc=<?php echo $ucc;?>">Awal</a>
						 </li>
						 <?php
						    $sm=$start-10;
							$ss=$start+10;
							...
							for ($i=$sm;$i<$ss;$i++)
							{
								...
								?>
									...
									<li class="page-item  <?php echo $active;?>" >
									    <a class="page-link" href="motorbaru.php?start=<?php echo $i;?>&kepala=-1&nama=<?php echo $uname1;?>&tipe=<?php echo $utipe;?>&harga=<?php echo $harga;?>&cc=<?php echo $ucc;?>"><?php echo ($i+1);?></a>
									</li>
								<?php
							}
						 ?>
						 <ul class="pagination pagination-lg justify-content-center">
						<li class="page-item " >
								  <a class="page-link" href="motorbaru.php?start=<?php echo $totalHalaman-1;?>&kepala=-1&nama=<?php echo $uname1;?>&tipe=<?php echo $utipe;?>&harga=<?php echo $harga;?>&cc=<?php echo $ucc;?>">Akhir</a>
						 </li>
					  </ul>
				</nav>
			</div>
				...
				<div class="card-body">
					<h5 class="card-title">============================================================================</h5>
						...
						if (count($dataN)>=1)
						{
							...
							echo "min cc : ". $dataN[0]["min_cc"]."<br/>";
							echo "max cc : ". $dataN[0]["max_cc"]."<br/>"."<br/>";
...
<script>
  ...
  function simpanpencarian()
  {
      ...
      console.log(param);
	  window.location="simpanhistorymotorbaru.php"+param;
  }
  function komaharga()
  {
	var harga=$("#harga").val();
